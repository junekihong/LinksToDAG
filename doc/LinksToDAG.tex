...
We publish our dictionary of label-to-direction assignments, that anyone can use to turn a link parse into a dependency parse.
...
We formulated an ILP to assign directional dependencies to all of the links in a set of link grammar parses, such that they are all connected DAGs, with the nodes reachable from the root. 
...
For example, setting the variables $$l_{\textsc{LEFT}} = \textsc{TRUE}, l_{\textsc{RIGHT}} = \textsc{FALSE}$$ would mean that the directional dependency of all links with label $l$ would be set to $\textsc{LEFT}$. If both variables were set to true, then the link type is allowed to go in either direction. Our ILP objective is to minimize the number of label\_direction variables set to true, while satisfying the other constraints.

The encoding was written in the Zimpl little language \cite{Koch2004} and solved using the SCIP Optimization Suite (cite?).
...
\begin{equation}
  count_{L, \textsc{left}} = \sum_{a \in \textsc{ARCS}_L} link(a,L)
\end{equation}
\begin{equation}
...
\caption{The ILP formulation. L is a label and M is the size of the corpus.}
