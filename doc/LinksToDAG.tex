...
\section{Data Sets}
The data used for this work came from English Penn Tree Bank CoNLL dependency training data \noteJH{cite?} 
%\noteJE{wait a minute, the bnews is not from CoNLL!  It is merely in CoNLL format.  It is from Ariya Rastrow, and is automatically parsed, I think.  We should be comparing on hand-parsed data.}  
and the Russian National Corpus\footnote{\url{http://ruscorpora.ru/en/}} \cite{russiancorpus} \noteJE{give citation}. 


We generated link parses using the AbiWord/CMU link grammar parser version 5.0.8 \cite{LINKPARSER-2014}.  The parser's coverage is not perfect: we obtained complete, connected parses for only \input{figure/conll_analysis_sentences_total.tex}of (\input{figure/conll_analysis_sentences_original.tex}) English sentences and only 206 (of 10,000) Russian sentences, discarding the other sentences. 
%\noteJE{fill these !!! in automatically} 
These two languages have the most complete lexicons at present, although lexicons for seven other languages are available.  
...
\NoteJE{give runtime here}\NoteJH{A run of 8,192 sentences takes 1743 seconds}
...
Of the English sentences with complete parses, \input{figure/conll_analysis_sentences_multiheaded}had at least one multiheaded word.
