...
In order to choose which direction the link labels are assigned, for every encountered label we generate two boolean label\_direction variables correspending to left and right. Setting the only one of these variables to \textsc{TRUE} chooses a consistent direction for a label throughout all parses, and because we assign a directionality to every link type at least one must be set to true. Setting both variables would allow us to go in either direction.

Our ILP objective is to minimize the number of label\_direction variables set to true, while still satisfying the acyclicity and reachability constraints. The encoding was written in the Zimpl little language \cite{Koch2004} and solved using the SCIP Optimization Suite (cite?).
...
%\centering
  \small
  \begin{align*}
    &\textsc{dir} = \{\textsc{left}, \textsc{right}\}, \textsc{l} \in \textsc{labels} & \\
  ...
  \begin{align*}
    \text{subject to } &\forall_{i \in \textsc{sentence}}: \textsc{reachable}_{\textsc{root}, i} = \textsc{true} &\\
                       &\forall_{\substack{i,j \in \textsc{sentence} \\ i\neq j }}: \textsc{cycle}_{i,j} = \textsc{false} &
  \end{align*}

  \caption{\small The ILP formulation.}
  %\\L is a label\\M is the size of the corpus.}
\end{figure}
