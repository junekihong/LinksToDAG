...
The English sentences were taken from the conll bnews training data (about 60,000 sentences)\noteJH{cite?}. We were also able to compare the resulting English link dependency data with the original conll to analyze the differences. 

In these languages, we also publish our link type to direction assignments so that anyone can use it to turn a link parse into a dependency parse.
...
Of the links that match a corresponding dependency arc 
...
\begin{figure}[ht!]
  ...
  \caption{\small The subsets of English sentences used for this work.}
\end{figure}

From the original sentences, we discarded disconnected parses for the ILP. From these sentences, we only looked at the complete link parses and discarded sentences with dropped words for our analysis section. We removed about \input{figure/conll_analysis_sentences_dropped} of the non-disconnected sentences this way.
