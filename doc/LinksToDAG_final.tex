...
% PREVIOUS ATTEMPT
% This does not rule out variation in the word order used to express a semantic relation.  For example, the English link grammar allows both SVO and VSO sentences, but with different edge labels.  An \texttt{S} edge encodes a ``subject-verb'' relation whose the left word serves as the subject, while an \texttt{SI} edge encodes the same relation when the the right word serves as the subject.  
...
\noteJE{check!}
One could experiment with adjusting the constant 1/4; we selected that value simply because it seemed like a reasonable threshold {\em a priori}.
% CUT FOR SPACE
% (i.e., $r_L+\ell_L=2$).  
