...
\begin{itemize}
\item This talk is about automatically converting from one annotation style to another.
\item The conversion could be hard, where information is fragmented or missing.

%This is often useful.  You may want to do it yourself.  
\item We use a general technique, Integer Linear Programming to recover this missing information.
\end{itemize}
}
...
\frame{\frametitle{Why We Want That}
  \begin{itemize}
...
\frame{\frametitle{Link Grammars}
  ...
  \item[] Orignal formalism and English Link Grammar created by Davy Temperley, Daniel Sleator, John Lafferty (1991)
  \end{itemize}
}

\subsection{Link Grammars Intro}
\frame{\frametitle{Link Grammars}
  \includegraphics[width=\linewidth]{linkgrammar_example1.png}\footnote{These figures were clipped from the orginal Link Grammar paper: \\``Parsing English with a Link Grammar'' by Sleator and Temperley}
}
...
\frame{\frametitle{Link Grammars: Same Example Parse Again}
  \begin{figure}
    \begin{dependency}[edge style={-}]
	  \begin{deptext}
	    - \& n-u \& v \& e \& e \& v \& v-d \& r \& n-u \& - \\
...
\frame{\frametitle{Link Grammars: Converting into a Directed Acyclic Graph Again}
  \begin{figure}
    \begin{dependency}
	  \begin{deptext}
	    - \& n-u \& v \& e \& e \& v \& v-d \& r \& n-u \& - \\
...
\begin{itemize}
  \item Its an optimization problem where some or all of the variables are integers. 
  \item The objective function and constraints are linear.
  \item Its NP-Hard! But good solvers exist that work well most of the time.
  \item Our ILP is encoded as a ZIMPL program and solved using the SCIP Optimization Suite
\end{itemize}
}
...
\begin{itemize}
  \item We start with a corpus of link grammar parses.
  \item We try to directionalize this corpus, choosing a direction for every link in every parse.
  \item We use an Integer Linear Program to find consistent directions for every link
\end{itemize}
}
...
Encoded Constraints:
\begin{itemize}
  \item Connectedness\uncover<2->{: Every word has a link attaching it}
  \item Acyclicity\uncover<2->{: No cycles!}
  \item Consistency of Directionalized Links\uncover<2->{:
...
\end{itemize}
\uncover<2->{\begin{center}
    A link can only be oriented left or right
\end{center}}
...
\frame{\frametitle{Stability of Results}
  ...
  \begin{itemize}
  \item We worried that the recovered direction mapping might be unstable and sensitive to the input corpus. 
...
\begin{itemize}
  ...
  \item The Link Grammar seems to be inconsistent about whether the auxiliary verb or the main verb is the head of a clause. 
  \item Sometimes the governing verb links to the auxilliary, and sometimes to the main, depending on the type of clause. 
  \item But the governing verb usually links to the subject when there is one.
  \item So this makes the subject a consistent choice to make the head of a clause.
\end{itemize}
